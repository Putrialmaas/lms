...
const storeData = (libraries, publishers, categories) => {
    form.library = library.value ? library.value.id : libraries[0].id;
    form.publisher = publisher.value ? publisher.value.id : publishers[0].id;
...
const chooseLibrary = (x) => {
    library.value = x;
};

const choosePublisher = (x) => {
    publisher.value = x;
};

const chooseCategory = (x) => {
    category.value = x;
};
...
<template>
    <AppLayout title="Books">
        ...
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            <FormSection @submitted="storeData(libraries, publishers, categories)">
                <template #title>Add Book</template>

                <template #description>
                    Add a new book data to database.
                </template>
